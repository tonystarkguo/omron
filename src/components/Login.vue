    ...
    <el-row>
      ...
      <el-col :xs="24" :sm="9" :md="9" :lg="9" :xl="9">
        <div class="logo-right"><div class="header"><img :src="LoginImgTopImg" alt=""><span>登陆</span></div>
          ...
          <el-form-item><el-button type="primary" size="medium" @click="submitForm('ruleForm2')">登录</el-button>
...
export default {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          // alert('submit!');
          const userId = 123;
          self.$router.push({ path: '/Home', params: { userId } });
        } else {
  ...
  created(){
    const obj={};
    const self=this;
    api.getLoginValue(obj).then(function(res){
      console.log(res);
      self.loginValueText=res;
      // self.loginValueText.splice(0,res.length)
